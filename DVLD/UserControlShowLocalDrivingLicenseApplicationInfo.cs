...
namespace DVLD
{
    public partial class UserControlShowLocalDrivingLicenseApplicationInfo : UserControl
    {
        int LDLAppID = -1;
        int ApplicantPersonID = -1;

        public UserControlShowLocalDrivingLicenseApplicationInfo()
        {
            InitializeComponent();
        }
        ...
        public void SetShowLicenseLinkLabelEnabled(bool value)
        {
            this.llShowLicenseInfo.Enabled = value;
        }
         ...
         public  void LoadData()
         {
            try
            {
                // Find the local  Application using LDLAppID
                var clsLDLApp = clsLocalDrivingLicenseApplicationsBusinessLayer.FindAppIDByLDLAppID(this.LDLAppID);
                if (clsLDLApp == null)
                {
              //      MessageBox.Show("The requested local Application was not found.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                // Find the associated Application
                var clsApplication = clsApplicationsBusinessLayer.FindApplication(clsLDLApp.ApplicationID);
        ...
        private void BindDataToControls(clsApplicationsBusinessLayer Application, clsLocalDrivingLicenseViewsBusinessLayer LocalAppView)
        {
            var usCulture = new CultureInfo("en-US"); // Setting the culture for formatting
            ...
            lblCreatedByValue.Text = clsUserBusinessLayer.FindUser(Application._CreatedByUser).UserName;
            ...
            lblType.Text = clsApplicationTypeBusinessLayer.FindApplications(Application._ApplicationTypeID).ApplicationTypeTitle;

            this.ApplicantPersonID = Application._ApplicantPersonID;
        }

        private void UserControlShowLocalDrivingLicenseApplicationInfo_Load(object sender, EventArgs e)
        {
        ...
        {
                FormShowLicense formShowLicense = new FormShowLicense(this.LDLAppID);
